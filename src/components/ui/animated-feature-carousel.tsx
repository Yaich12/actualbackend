...
function FeatureCard({ children, step, steps: stepsData }: { children: React.ReactNode; step: number; steps: readonly Step[] }) {
  ...
  return (
    ...
    >
      <div className="relative w-full overflow-visible rounded-3xl bg-transparent transition-colors duration-300">
                    ...
                    return (
                        <motion.li key={step.name} initial="inactive" animate={isCurrent ? "active" : "inactive"} variants={stepVariants} transition={{ duration: 0.3 }} className="relative" >
                            <button
                                type="button"
                                className={cn(
                                    "group flex items-center gap-2 text-sm font-medium text-neutral-800 transition-colors duration-200 dark:text-neutral-200",
                                    isCurrent ? "underline underline-offset-4 text-sky-700 dark:text-sky-300" : "hover:text-sky-700 dark:hover:text-sky-300"
                                )}
                                onClick={() => onChange(stepIdx)}
                            >
                                <span className={cn(
                                    "flex h-5 w-5 shrink-0 items-center justify-center text-xs transition-opacity duration-200",
                                    isCurrent ? "opacity-100 text-sky-700 dark:text-sky-300" : "opacity-70 group-hover:opacity-100"
                                )}>
                                    {isCompleted ? (
...
const defaultClasses = {
  ...
  step1img1: "w-[55%] right-[-35%] top-[-40%]",
  step1img2: "hidden",
  step2img1: "w-[55%] right-[-35%] top-[-40%]",
  step2img2: "hidden",
  step3img: "w-[55%] right-[-35%] top-[-40%]",
  step4img: "w-[55%] right-[-35%] top-[-50%]",
} as const
...
export function FeatureCarousel({
  ...
  const { currentNumber: step, setStep } = useNumberCycler(stepsToUse.length)
  ...
  return (
    <div className="flex flex-col gap-12 w-full max-w-4xl mx-auto p-4">
        ...
        {/* Navigation hidden per request; auto-cycling still works */}
    </div>
  )
}
